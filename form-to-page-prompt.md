# FiveSeriesHQ — move submit form to its own page (/technical/submit)

Read `CLAUDE.md` first. **Do NOT touch the database, do NOT change section values / table
names / function names, do NOT commit or push.** This session has no git push auth — make
the edits and stop. The user commits/pushes via GitHub Desktop and tests the Vercel
preview. Do NOT run `npm run build` (the "My Drive" path breaks it).

This is a frontend-only refactor. App Router project. Goal:
1. Extract the submit form logic out of `TechSubmitModal.tsx` into a new reusable
   `TechSubmitForm.tsx` (no popup chrome — renders as a plain page section).
2. Create a page at `src/app/technical/submit/page.tsx` that reads `gen` and `section`
   from the URL and renders `<TechSubmitForm>`.
3. In `TechnicalInfo.tsx`, change every "Submit" trigger from opening the modal to
   navigating to `/technical/submit?gen=...&section=...`. Remove the modal usage and the
   `showSubmitModal` state. **Leave `TechSubmitModal.tsx` on disk** (just no longer
   imported/used).

Do NOT modify the `useEffect` blocks or data-loading logic in `TechnicalInfo.tsx` — only
the submit buttons, the modal render, and the now-unused state.

---

## File 1 — create `src/components/technical/TechSubmitForm.tsx`

This is the existing modal's form, with the popup wrapper removed. It takes `defaultGen`
and `defaultSection` props and an `onDone` callback. Create it with this content:
...
## File 2 — create `src/app/technical/submit/page.tsx`

Create the directory `src/app/technical/submit/` if needed. This page reads `gen` and
`section` from the URL and renders the form. It uses Suspense because `useSearchParams`
...
export default function SubmitPage() {
  return (
    <Suspense fallback={<div className="h-96 bg-gray-50 animate-pulse rounded-xl max-w-2xl mx-auto my-8" />}>
      <SubmitPageInner />
    </Suspense>
  )
}
...
## File 3 — edit `src/components/technical/TechnicalInfo.tsx`

Make ONLY these changes. Do not touch the useEffect blocks or data-loading logic.

### (a) Remove the modal import
Find and delete this line:
```tsx
import { TechSubmitModal } from './TechSubmitModal'
```

### (b) Remove the modal state
Find and delete this line:
```tsx
  const [showSubmitModal, setShowSubmitModal] = useState(false)
```

### (c) Add a helper for the submit href (place it right after the `genColors` line)
Find:
```tsx
...
### (d) Article-detail view: the GenSidebar onSubmit
Find (in the article-detail return block):
```tsx
...
### (h) Documents empty-state onSubmit
Find:
```tsx
...
### (i) System-list empty-state onSubmit
Find:
```tsx
              <EmptyState
...
### (j) Remove the main-view modal render
Find and delete:
```tsx
      {showSubmitModal && (
        <TechSubmitModal
...
### (k) Update GenSidebar to take an href instead of a click handler
Change the `GenSidebar` function signature. Find:
```tsx
...
```
Then find the submit button inside GenSidebar:
```tsx
      {user && (
        <div className="pt-2">
...
### (l) Update EmptyState to take an optional href instead of a click handler
Find:
```tsx
...
(`Link` is already imported in `TechnicalInfo.tsx`, so no import change is needed there.)

---

## When done
Make the edits and STOP. Do not commit, push, or run the build. Report what you changed.
Note: `TechSubmitModal.tsx` is intentionally left on disk and unused.
